...
class AI:
    ...
    def minimax(self, game_state, depth, alpha, beta, maximizing_player):
        ...
        if maximizing_player:
            ...
            for action in self.get_possible_actions(game_state, game_state.current_player):
                ...
                score, _ , sequence = self.minimax(new_game_state, depth - 1, alpha, beta, False)
                if score > max_score:
                    max_score = score
                    best_action = action
            ...
            return max_score, best_action, [(max_score, best_action)] + best_sequence
        else:
            min_score = float('inf')
            best_action = None
            best_sequence = None
            for action in self.get_possible_actions(game_state, game_state.current_player):
                new_game_state = copy.deepcopy(game_state)
                new_game_state.apply_action(action)
                score, _, sequence = self.minimax(new_game_state, depth - 1, alpha, beta, True)
                if score < min_score:
            ...
            return min_score, best_action, [(min_score, best_action)] + best_sequence
    ...
    def iterative_deepening_minimax(self, game_state, max_depth):
        ...
        for depth in range(1, max_depth + 1):
            ...
            score, action, sequence = self.minimax(game_state, depth, float('-inf'), float('inf'), True)
        ...
        # print sequence
        for elem in best_sequence:
            print(f"Score: {elem[0]}, Action: {elem[1]}")
